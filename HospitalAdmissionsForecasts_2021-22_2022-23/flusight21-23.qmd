...
include21 <- all_dat21 %>% 
  ...
  mutate(per_forecasts = round((n_forecasts/(baseline_n_forecasts))*100,2),
         per_locations = round((n_locations/length(unique(dat_for_scores21$location)))*100,2)) %>% 
...
include23 <- all_dat23 %>% 
  ...
  mutate(n_forecasts = sum(!is.na(value)),
         n_locations = length(unique(location))) %>%
  mutate(per_forecasts = round((n_forecasts/(baseline_n_forecasts))*100,2),
         per_locations = round((n_locations/length(unique(dat_for_scores23$location)))*100,2)) %>% 
  filter(per_forecasts >= 75) %>%
  select(model,n_forecasts, per_forecasts, n_locations, per_locations) %>% 
  distinct() %>% ungroup()
...
dat_for_scores21 <- dat_for_scores_function(all_dat21, obs_data21) %>% 
  ...
  filter(model %in% include21$model)
dat_for_scores23 <- dat_for_scores_function(all_dat23, obs_data23) %>% 
 rename("prediction" = "value", "true_value" = "report") %>%
  filter(model %in% include23$model)
...
model_order <- merge(inc.rankings_all_nice[,1:3], inc.rankings_location, by = "model", all.y = TRUE) %>% arrange(rel_wis)
