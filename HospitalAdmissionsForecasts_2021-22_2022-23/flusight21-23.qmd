...
#model names are read in from the below script
suppressMessages(invisible(source(paste0(manuscript_repo,"/Model names and colors.R"))))
#sources functions used in qmd
source(paste0(manuscript_repo,"/functions2022-2023.R"))

#ensure we are using dplyr select and filter without specifying every single time
select = dplyr::select
filter = dplyr::filter
...
#separate 21-22 and 22-23 data using specified season dates; remove Virgin Islands, and keeps only unique forecasts if more than one was accidentally read in
...
##read in observed data from 9/12/22 and 6/23/23 for the 21-22, and 22-23 season respectively. Filter out VI
...
#function below needs all_dat by location name, this pulls out location and location name and joins it with all_dat
...
WIS_all21 <- raw_scores21 %>% summarise_scores(na.rm = TRUE) %>% mutate(wis=round(interval_score,2))
...
WIS_all23 <- raw_scores23 %>% summarise_scores(na.rm = TRUE) %>% mutate(wis=round(interval_score,2))
...
ggsave(paste0(manuscript_repo,"/Output/figure1a.png"), plot = figure1a, width=8, height=6)
ggsave(paste0(manuscript_repo,"/Output/figure1b.png"), plot = figure1b, width=8, height=6)
...
ggsave(paste0(manuscript_repo,"/Output/Figure_1.pdf"), plot = figure1, width=12, height=6)
...
inc.rankings_all <- rbind(mutate(inc.rankings_all21, season = "2021-2022"), mutate(inc.rankings_all23, season = "2022-2023")) %>% arrange(season, rel_wis)
 ...
 logtable1 <- read.csv(paste0(manuscript_repo,"/Supplemental_analyses/log-transformed/logtable1.csv")) 
  ...
  inc.rankings_all %>% 
    left_join(., logtable1, join_by("model" == "Model", "season" == "Season")) %>% 
    rename(Model = model,
           `Absolute WIS` = wis,
    ...
    kableExtra::kable_classic(html_font = "Arial", full_width = F, ) %>% 
     kableExtra::column_spec(1, width_min = "2cm") %>% 
     kableExtra::column_spec(2:9, width_min = "1cm") %>% 
    kableExtra::save_kable(file = paste0(manuscript_repo,"/Output/Table_1test.pdf"), density = 320)
...
### previous method  
#WIS_Season21 <- WIS_Season21 %>% filter(model %in% include21$model)
...
#WIS_Season23 <- WIS_Season23 %>% filter(model %in% include23$model)
...
`r nrow(filter(inc.rankings_all21, rel_wis < 1))` models in 2021-2022 and `r nrow(filter(inc.rankings_all23, rel_wis <1))` models in 2022-2023 outperformed the naive FluSight baseline in relative WIS over the evaluation period (Table 1). 
...
ggsave(paste0(manuscript_repo,"/Output/Figure2.pdf"), width=8, height=8, units="in", plot = figure2)
...
figure3 <- ggplot(scores, 
  ...
  geom_tile() +
  theme_bw()+
  geom_text(aes(label = signif(rel_wis, 2)), size = 1.5) + # I adapted the rounding 
...
figure3
# ggsave(paste0(manuscript_repo,"/Output/Figure_3.pdf"), plot = figure3, width = 11, height= 8, dpi = 320)
# ggsave(paste0(manuscript_repo,"/Output/Figure_3test.pdf"), plot = figure3, width = 8, height= 5.8, dpi = 320)


modelrankings21 <- inc.rankings_location21 %>%  ungroup()%>% group_by(model) %>% summarise(low = min(rel_wis), high = max(rel_wis), median = median(rel_wis), mean = mean(rel_wis)) %>% mutate(diff = high - low)

modelrankings23 <- inc.rankings_location23 %>%  ungroup()%>% group_by(model) %>% summarise(low = min(rel_wis), high = max(rel_wis), median = median(rel_wis), mean = mean(rel_wis)) %>% mutate(diff = high - low)
...
Model performance varied by spatial jurisdiction, with state-specific relative WIS values with similar ranges each year with `r round(min(inc.rankings_location21$rel_wis), 2)` to `r round(max(inc.rankings_location21$rel_wis), 2)` in 2021-22 and `r round(min(inc.rankings_location23$rel_wis), 2)` to `r round(max(inc.rankings_location23$rel_wis), 2)` in 2022-23. The relative WIS of the FluSight Ensemble had the smallest range of values across all locations from `r filter(inc.rankings_location21, model == "Flusight-ensemble") %>% {round(min(.$rel_wis),2)}` to `r filter(inc.rankings_location21, model == "Flusight-ensemble") %>% {round(max(.$rel_wis),2)}` in 2021-22 to `r filter(inc.rankings_location23, model == "Flusight-ensemble") %>% {round(min(.$rel_wis),2)}` to `r filter(inc.rankings_location23, model == "Flusight-ensemble") %>% {round(max(.$rel_wis),2)}` in 2022-23 (Figure 4 and Supplemental Figure X boxplot). To further examine forecast performance across jurisdictions, we considered the percent of jurisdictions that the relative WIS value for a given model and location pair was less than the baseline (i.e., lower than 1). The FluSight Ensemble performed better than the baseline for `r nrow(filter(inc.rankings_location21, model == "Flusight-ensemble" & rel_wis <1))` out of `r nrow(inc.rankings_location21 %>% filter(model == "Flusight-ensemble"))` forecast jurisdictions. More models performed better than the baseline in 2022-23 than 2021-22. In 2021-22, `r nrow(filter(Scores_tab21, model != "Flusight-ensemble", below_baseline_pct >= 50))` models performed better than the baseline at least 50% of the time, compared to `r nrow(filter(Scores_tab23, model != "Flusight-ensemble", below_baseline_pct >= 50))` .
...
abs_states <- WIS_Season %>% filter(location_name != "National") %>% 
  ...
  summarise(abs_WIS = mean(WIS,na.rm = TRUE)) %>% 
...
figure4 <-  ggplot(abs_flusight, aes(x = target_end_date,
  ...
  facet_grid(rows = vars(target), cols = vars(season), labeller = wis_labels,  scales = "free_x")
figure4
# ggsave(paste0(manuscript_repo,"/Output/Figure_4.png"), plot = figure4, width = 8, height = 5)
# ggsave(paste0(manuscript_repo,"/Output/Figure_4.pdf"), plot = figure4, width = 8, height = 5)
...
coverage95_summary <- WIS_Season %>%  filter(model == "Flusight-ensemble") %>% filter(location_name != "National") %>%
  ...
  summarise(model = model,
            ...
            coverage95 = sum(coverage.95, na.rm =TRUE)/locationcount) %>% ungroup() %>% unique()
...
coverage50_summary_4 <- WIS_Season %>%  filter(model == "Flusight-ensemble") %>% filter(location_name != "National") %>%
  ...
  summarise(model = model,
            forecast_date = as.Date(forecast_date, format = "%Y-%m-%d"),
            coverage50 = sum(coverage.50, na.rm = TRUE)/locationcount) %>% unique()
...
coverage50_summary <- WIS_Season %>% filter(model == "Flusight-ensemble") %>% filter(location_name != "National") %>%
  ...
  summarise(model = model,
            forecast_date = as.Date(forecast_date, format = "%Y-%m-%d"),
            coverage50 = sum(coverage.50, na.rm = TRUE)/locationcount) %>% ungroup() %>% unique()
...
coverage95_summary_all <- WIS_Season %>%  filter(model != "Flusight-ensemble") %>% filter(location_name != "National") %>%
  ...
  summarise(model = model,
            forecast_date = as.Date(forecast_date, format = "%Y-%m-%d"),
            coverage95 = sum(coverage.95, na.rm =TRUE)/locationcount) %>% unique() %>% 
...
coverage50_summary_all <- WIS_Season %>% filter(model != "Flusight-ensemble") %>% filter(location_name != "National") %>%
  ...
  summarise(model = model,
            forecast_date = as.Date(forecast_date, format = "%Y-%m-%d"),
            coverage50 = sum(coverage.50, na.rm = TRUE)/locationcount)
...
cov95_breakdownall %>% arrange(season, Relative_WIS) %>% mutate_if(is.numeric, round, digits = 2) %>% select(-season) %>%  
  knitr::kable(align = c("lcccccccccc"), #caption = "Table 2", 
               col.names = c("Model", "Relative WIS", "% WIS Below Baseline", "1 Wk Coverage", "2 Wk Coverage", "3 Wk Coverage", "4 Wk Coverage", "% Cov abv 90 (1 Wk)", "% Cov abv 90 (2 Wk)", "% Cov abv 90 (3 Wk)", "% Cov abv 90 (4 Wk)")) %>% 
 ...
 # kableExtra::kable_classic()  %>%    kableExtra::save_kable(file = paste0(manuscript_repo, "/Output/Table_2.pdf"))
...
abs_states_all <- WIS_Season %>% filter(location_name != "National") %>% 
  ...
  summarise(model = model,
            ...
            abs_WIS = mean(WIS,na.rm = TRUE))


model_abs_all <- abs_states_all %>% unique() %>% group_by(model, target, season) %>% summarise(
  model = model,
  target = target,
  minimum = min(abs_WIS,na.rm = TRUE),
  maximum = max(abs_WIS,na.rm = TRUE),
  Median = median(abs_WIS,na.rm = TRUE)
) %>% unique()

abs_breakdown <- WIS_Season %>% group_by(model, season) %>% summarise(
  model = model,
  One_week_abs = mean(WIS[target == "1 wk ahead inc flu hosp"],na.rm = TRUE),
  Two_week_abs = mean(WIS[target == "2 wk ahead inc flu hosp"],na.rm = TRUE),
  Three_week_abs = mean(WIS[target == "3 wk ahead inc flu hosp"],na.rm = TRUE),
  Four_week_abs = mean(WIS[target == "4 wk ahead inc flu hosp"],na.rm = TRUE)
) %>% unique() 
...
abs_breakdown_WIS %>% select(-season) %>% mutate_if(is.numeric, round, digits = 2) %>%  
  knitr::kable(align = c("lccccc"), #caption = "Table 3", 
               col.names = c("Model", "Relative WIS", "1 Wk ABS WIS", "2 Wk ABS WIS", "3 Wk ABS WIS", "4 Wk ABS WIS")) %>% 
 # kableExtra::footnote(general = "Table 3", general_title = "")%>% 
  kableExtra::pack_rows(index = table(abs_breakdown_WIS$season)) %>% 
  kableExtra::kable_classic() %>%   kableExtra::save_kable(file = paste0(manuscript_repo, "/Output/Table_3.pdf"))
...
figure5 <- ggplot(coverage95_flusight, aes(x = target_end_date, 
...
figure5
 ...
 ggsave(paste0(manuscript_repo,"/Output/Figure_5_colors.pdf"), width=8, height=6, plot = figure5)
...
model_order <- merge(inc.rankings_all_nice[,c(1,12)], inc.rankings_location, by = "model", all.y = TRUE) %>% arrange( rel_wis, season)

figs1 <- model_order %>% 
  ggplot( aes(x = fct_inorder(model), y = rel_wis))+
  geom_boxplot()+
  theme_bw() +
  ...
  facet_grid(cols = vars(season), scales = "free_x", labeller = as_labeller(c(`2021-2022` = "2021-2022", `2022-2023` = "2022-2023")))
figs1
 # ggsave(paste0(manuscript_repo,"/Output/Figure_S1.jpg"), plot = figs1, width=8, height=6)
```
