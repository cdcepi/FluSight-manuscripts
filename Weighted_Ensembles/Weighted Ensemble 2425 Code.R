...
flu_dates_24_25 <- as.Date("2024-11-23") + weeks(6:27)#Starting 6 weeks later than the start of forecast (burn period)
flu_dates_24_25 <- flu_dates_24_25[flu_dates_24_25 != as.Date("2025-01-25")] #Omit 2025-01-25 since forecasts were paused that week

# Get all eligible dates that are earlier than the current reference date
eligible_dates <- as.Date("2024-11-23") + weeks(0:27)
eligible_dates <- eligible_dates[eligible_dates != as.Date("2025-01-25")]
...
for (j in 1:length(flu_dates_24_25)) {
  current_ref_date<-flu_dates_24_25[j]
  forecasts_6week<-data.frame()
  
  #This the period of time we will use to evaluate performance
  # flu_dates_24_25_retro <- as.Date(current_ref_date) - weeks(1:6)
  earlier_dates <- eligible_dates[eligible_dates < current_ref_date]
  
  # Select the last 6 available dates
  flu_dates_24_25_retro <- tail(earlier_dates, 6)
  ...
  for (i in 1:length(flu_dates_24_25_retro)) {
    ...
    eligible_models = read.csv("Data/model-eligbility/models-to-include-in-ensemble-202425.csv")
  ...
  eligible_models = read.csv("Data/model-eligbility/models-to-include-in-ensemble-202425.csv")%>% filter(Model!="FluSight-baseline")
  models = as.character(eligible_models$Model)
...
rm(weighted_US_weights,weighted_state_weights,weighted_US,weighted_state)

save(weighted_state_forecasts_all, file="Data/rds/2425/weighted state forecasts 2425.rds")
save(weighted_US_forecasts_all,file="Data/rds/2425/weighted US forecasts 2425.rds")

save(weighted_state_weights,file="Data/rds/2425/weighted state weights 2425.rds")
save(weighted_US_weights,file="Data/rds/2425/weighted US weights 2425.rds")


load(file="Data/rds/2425/weighted state forecasts 2425.rds")
load(file="Data/rds/2425/weighted US forecasts 2425.rds")

load(file="Data/rds/2425/weighted state weights 2425.rds")
load(file="Data/rds/2425/weighted US weights 2425.rds")


#Scoring ensembles
#First bringing in ensemble, baseline, and LOP
...
all_metadata = file_names[!grepl(paste0(".md", collapse = "|"), file_names)]# %>%
...
eligible_models <- data.frame(Model = file.names, Designated_Model = designated_models) %>% 
  filter(Designated_Model == T |Model=="FluSight-baseline"|Model=="FluSight-ensemble")
...
all_forecasts_us_compl2425<-all_forecasts_us_compl
all_forecasts_state_compl2425<-all_forecasts_state_compl


save(all_forecasts_us_compl2425,file="Data/rds/2425/all_forecasts_us_compl2425.rds")
save(all_forecasts_state_compl2425,file="Data/rds/2425/all_forecasts_state_compl2425.rds")


load(file="Data/rds/2425/all_forecasts_us_compl2425.rds")
load(file="Data/rds/2425/all_forecasts_state_compl2425.rds")  
...
weighted_state_forecasts_all2<-rbind.fill(weighted_state_forecasts_all,all_forecasts_state_compl2425)%>%unique()%>%left_join(flu_truth_current %>%  dplyr::select(target_end_date , location, true_value=value),
                                                            by = c("location", "target_end_date"))%>%dplyr::rename(prediction=value)%>%
...
weighted_US_forecasts_all2<-rbind.fill(weighted_US_forecasts_all,all_forecasts_us_compl2425)%>%unique()%>%left_join(flu_truth_current %>%  dplyr::select(target_end_date , location, true_value=value),
                                                            by = c("location", "target_end_date"))%>%dplyr::rename(prediction=value)%>%
  dplyr::mutate(model=model_id,quantile=output_type_id)%>%dplyr::select(-model_id, -output_type_id,-target )%>%mutate(quantile=as.numeric(quantile))
...
test_truth2<-test_truth%>%mutate(target_end_date=as.Date(target_end_date), target_variable='wk inc flu hosp')
weighted_US_forecasts_all3<-weighted_US_forecasts_all2%>%select(-target_variable,-model,-quantile) %>%as_covid_hub_forecasts()
weighted_state_forecasts_all3<-weighted_state_forecasts_all2%>%select(-target_variable,-model,-quantile) %>%as_covid_hub_forecasts()


weighted_US_forecasts_all3_trunc<-weighted_US_forecasts_all3%>%
  filter(forecast_date >= as.Date("2024-12-07") & forecast_date <= as.Date("2025-04-12"))

weighted_state_forecasts_all3_trunc<-weighted_state_forecasts_all3%>%
  filter(forecast_date >= as.Date("2024-12-07") & forecast_date <= as.Date("2025-04-12"))


us_score_test<-covidHubUtils::score_forecasts(weighted_US_forecasts_all3,test_truth2,use_median_as_point = T,return_format="wide")

weighted_fore_eval_us <- us_score_test |>
  evaluate_flu_scores(grouping_variables=NULL, baseline_name="FluSight-ensemble", us_only=TRUE)


state_score_test<-covidHubUtils::score_forecasts(weighted_state_forecasts_all3_trunc,test_truth2,use_median_as_point = T,return_format="wide")

weighted_fore_eval_state <- state_score_test |>
  evaluate_flu_scores(grouping_variables=NULL, baseline_name="FluSight-ensemble", us_only=F)


#Scored forecasts
weighted_state_score_all<-rbind(weighted_state_score_all,weighted_state_score)

wis_model_us_2425 <- weighted_US_forecasts_all2 %>%filter(is.na(true_value)==F) %>%
...
wis_model_states_2425 <- weighted_state_forecasts_all2%>%
  select(c(true_value, location, prediction, quantile, model,horizon,reference_date))%>%check_forecasts()%>%score%>%
  add_coverage(ranges = c(50, 95), by = c("model")) %>%
...
wis_model_us_2425_trunc <- weighted_US_forecasts_all2 %>%
  select(c(true_value, location, prediction, quantile, model,horizon,reference_date))%>%
  filter(reference_date >= as.Date("2024-01-04") & reference_date <= as.Date("2025-04-19"))%>%
...
wis_model_states_2425_trunc <- weighted_state_forecasts_all2%>%
  select(c(true_value, location, prediction, quantile, model,horizon,reference_date))%>%
  filter(reference_date >= as.Date("2024-01-04") & reference_date <= as.Date("2025-04-19"))%>%
  check_forecasts()%>%score%>%
  add_coverage(ranges = c(50, 95), by = c("model")) %>%
...
save(wis_model_states_2425,file="Data/rds/2425/wis_model_states_2425.rds")
save(wis_model_us_2425,file="Data/rds/2425/wis_model_us_2425.rds")

save(wis_model_states_2425_trunc,file="Data/rds/2425/wis_model_states_2425_trunc.rds")
save(wis_model_us_2425_trunc,file="Data/rds/2425/wis_model_us_2425_trunc.rds")
