  ...
  if ("season" %in% grouping_variables & isFALSE("season" %in% names(scores))) {
    scores <- scores |>
      dplyr::mutate(season = case_when(
        forecast_date < as.Date("2022-08-01") ~ "2021-2022",
        forecast_date >= as.Date("2022-08-01") & forecast_date < as.Date("2024-07-01") ~ "2022-2023",
        forecast_date >= as.Date("2024-07-01") ~ "2023-2024"
      ))
  }
