...
#'
#' @examples
as_covid_hub_forecasts2 <- function(model_outputs, reference_date_col="reference_date", location_col="location", horizon_col="horizon", target_col="target_variable", temp_res_col="temporal_resolution", target_end_date_col="target_end_date") {

  model_outputs <- model_outputs |> 
    dplyr::rename(model = model_id, type = output_type, quantile = output_type_id,
                  reference_date = reference_date_col, location = location_col, target_variable = target_col) 
  ...
  if (is.null(target_end_date_col)) {
    model_outputs <- model_outputs |>
      dplyr::mutate(target_end_date=case_when(
        temporal_resolution %in% c("d", "day") ~ reference_date + lubridate::days(horizon),
        temporal_resolution %in% c("w", "wk", "week") ~ reference_date + lubridate::weeks(horizon),
        temporal_resolution %in% c("m", "mth", "mnth", "month") ~ reference_date %m+% months(horizon),
        temporal_resolution %in% c("y", "yr", "year") ~ reference_date + lubridate::years(horizon),
        .default = reference_date), 
      .before = type) 
  }
  
  covid_hub_outputs <- model_outputs |>                
    dplyr::select(model, reference_date, location, horizon, temporal_resolution, 
